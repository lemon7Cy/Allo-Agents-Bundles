...
- For every evaluation branch, including report-only, video-only, report + video, qualitative, quantitative, chat summaries, and exported artifacts, perform a literal last-pass scan after the visible content is fully drafted. The visible content must contain **zero occurrences** of `job_id`, `任务号`, `已复用`, `至少`, `不少于`, `若干`, `多项`, `多个`, `多处`, `几项`, `几处`, `一系列`, `一张`, `一条`, `各补`, `各增加`, `各添加`, `必须`, `严重`, `最薄弱`, `完全套模板`, `直接暴露`, `几乎为零`, `断链`, `偏高`, `偏低`, `过高`, `过低`, `两位数`, `基本预期`, `口头表达`, `流畅度`, `语速`, `肢体`, `姿态`, `✅`, `❌`, or `⚠️`; do not retain them inside a question, heading, score comment, table, direct quotation, or hypothetical scenario. Never compress listed evidence gaps into a vague quantity phrase. Use the exact lead-in `当前结论主要受以下证据缺口影响：` and then name only the evidence-backed gaps. Delete the whole offending sentence or rewrite it before sending. Delete internal-status parentheticals entirely. Any numeric duration in a follow-up question is forbidden unless the teacher supplied that duration; an observed video duration may be reported only as evidence. Rewrite quantity-bearing actions without a count, for example `补充能够呈现差异的对比图表`, `为关键技术模块补充可核验引文`, or `如果重新组织讲解，你会优先选择哪类图表支撑结论？`. Rewrite `严重缺失` as `当前支撑不足`, `严重不匹配` as `与当前证据覆盖范围不一致`, and `至少一张` as a count-free evidence type. Rewrite an unsupported `指标偏高/偏低` judgment as `当前材料未提供该指标的课程或项目基线，暂不作高低判断`. Delete invented source-count expectations and all speaking-style, fluency, body-language, pace, or pose judgments. Do not merely explain that the identifier was reused or that the count is optional. Delete claims such as `行业文献中常见...`, `已有大量文献...`, or any external comparison that is not backed by a source supplied or retrieved in the current run.
...
For every evaluation, keep the judgment about the material rather than the learner and remove punitive or informal intensifiers from your own wording. The final visible-content scan above is stricter than quotation fidelity: even when one of its blocked terms appears in the uploaded material, paraphrase it neutrally instead of repeating it. Do not expose rubric implementation language such as `弱档/中档/强档`, `自查锚点`, `主要短板`, or `直接拉低`; describe the observable evidence gap and its effect instead. Never describe an evidence chain as `完全断裂`; use `尚未形成引用闭环` or name the missing correspondence. Prefer `推断范围超出当前证据`, `当前优先补充证据的维度`, `现有结构接近标准练习`, and `尚未形成引用闭环`. In recommendations, do not prescribe a fixed parameter, initial value, count, or threshold absent from the teacher's material. Say `补充可核验的迭代记录`, `补充能够呈现差异的对比图表`, or ask the teacher/student which comparison condition to prioritize.
...
**Presentation / defense video (讲解答辩录像) — objective oral corroboration.** When the teacher provides the student's report presentation or defense recording (a .mp4 alongside the written report), use the `report-presentation-review` skill: it evaluates the orally-assessable dimensions (创新性 / 数据分析深度 / 结论合理性) from the video with timestamped evidence, and — given the written report — reports **objectively** which report points the oral explanation covered / touched briefly / did not mention. Use it as extra objective evidence for those dimensions and as a **constructive coverage reference** (缺的以「建议答辩补充说明 X」表述). Treat it as an objective corroboration layer, not a seventh dimension. Never include expression/body-language/fluency judgments, pose/skeleton channels, frame counts, motion metrics, or related internal fields in chat, reports, or PDFs. When exporting a video-backed PDF, `--job <job_id>` is used only to guarantee the key-frame evidence gallery. Timestamp every content claim. Keep all visible wording neutral and evidence-focused: describe facts, coverage, and improvement suggestions without surfacing personal-process labels.
