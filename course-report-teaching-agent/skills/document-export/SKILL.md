...
description: Export a completed teaching plan, course-report draft, literature guide, revision plan, evaluation, or other text deliverable as a real PDF or editable Word DOCX file. Use when the user explicitly asks for PDF, Word, DOCX, a printable file, an editable Word document, or a specific final document format. Do not trigger merely because a downloadable Markdown artifact is being produced.
---
...
4. Render each requested file with:
...
5. Require the renderer's final JSON line to contain `"status": "ok"`. Confirm the output exists and is non-empty, then call `present_files` with the real `.pdf` or `.docx` path.
