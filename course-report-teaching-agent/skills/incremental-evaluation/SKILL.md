...
Before rendering a report-based PDF, run `scripts/validate_evaluation_evidence.py` with `python3` against the JSON and source report Markdown. The validator loads the adjacent bundled `rubric.md` itself, so do not pass a second `/mnt/skills/agent/...` rubric path. When the PDF includes video evidence, save the authoritative `course-eval` result to `/mnt/user-data/tmp/course-eval.json` and add `--video-evidence /mnt/user-data/tmp/course-eval.json`; this grounds timestamps, key-frame paths, and other video-derived facts without weakening report-score validation. Fix a failed validation from current evidence; never bypass it, install packages, or continue to rendering after a blocked/failed validation.
