...
这是教学/评价时拿**真实文献证据**的检索工具(明学库覆盖电池/储能/SOC/SOH/RUL/Kalman/BMS 等)。

- **选题/材料支撑**:推荐选题、整理语料库时,用本库检索真实文献给学生指方向,而不是空泛建议。
- **核查学生引用(评价前哨)**:评审"文献引用"维度时,把学生引用的观点拿来本库检索,核对**是否真实存在、是否支撑其结论**——正是 `literature-and-knowledge-guide` 里"虚构引用/引用不支撑观点"那几条的取证手段。
- **限于覆盖域**:选题不在本库覆盖域(电池/储能等)时,回到通用文献方法,别硬套。
- **不杜撰**:只引用返回块里真实的文档;查不到标"待核实"。
