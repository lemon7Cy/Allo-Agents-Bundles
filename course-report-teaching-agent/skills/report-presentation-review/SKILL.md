...
- `oral_assessable_dimensions[]` — for 创新性 / 数据分析深度 / 结论合理性: `level` (强/中/弱/证据不足) + `comment` + timestamped `evidence`. **Use these to corroborate the written six-dimension scores** with oral evidence. If the report scores a dimension high but the oral level is 弱/证据不足, note it factually as "口头证据偏少,建议答辩补充".
