...
- **`key_frames`** (per dimension) — each has `timecode`, `why`, and a **`frame_path`**: an actual video frame that `course-eval` has already **saved as an image file** under `/mnt/user-data/outputs/关键帧证据/`. **When you export the PDF, feed these into a 「关键帧证据」 `gallery` block** (see [[report-pdf-export]]) so the teacher SEES the frame behind each judgment — this is a required part of a video-based evaluation PDF, not optional. Use the `frame_path` directly; do not re-encode.
