...
- If an official page cannot be opened, is too shallow to support the needed fields, or its metadata does not match, do not retry that URL and do not open another view of the same DOI/title. Keep the item under `检索线索（当前未核实）` or replace it with another candidate; do not fill the missing fields and do not summarize methods/results as established facts.
