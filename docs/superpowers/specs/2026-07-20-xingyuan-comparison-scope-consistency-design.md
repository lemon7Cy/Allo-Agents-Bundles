...
When validation fails, the engine returns a non-zero exit code and a bounded actionable error identifying the mismatched field and periods. This rejection is an internal guard, not a normal final answer. The agent must discard all incompatible cached or previous results, lock the user's requested metric, scope, timezone, cutoff, date semantics, period duration, and one roster snapshot, then re-query every period from MCP with identical `group_by=user` and hour cutoff. It rebuilds the complete input and reruns the engine exactly once. The rule is: do not patch only one period and do not reuse stale data. There is one standardized full re-query maximum.

If that retry succeeds, the successful retry is answered normally without mentioning the internal rejection. Only if the standardized re-query itself fails or returns incomplete data may the agent respond with:

```text
状态：数据不足
诊断：标准化完整重取数失败或数据不完整，已停止计算环比。
缺口：<concrete missing MCP calls, periods, fields, or failure reasons>。
```
...
- Assert that `SOUL.md` and `SKILL.md` require one full standardized MCP re-query before any final insufficient-data response, forbid single-period patching and stale-data reuse, and hide a successful internal recovery.
...
- A scope-validation rejection triggers discard of all incompatible cached or previous results and re-query every period from MCP with one locked scope contract.
- Recovery has one standardized full re-query maximum; do not patch only one period or reuse stale data.
- A successful retry is answered normally without mentioning the internal rejection.
- Final insufficient-data output is allowed only when the standardized re-query fails or is incomplete, and it names concrete missing MCP calls.
