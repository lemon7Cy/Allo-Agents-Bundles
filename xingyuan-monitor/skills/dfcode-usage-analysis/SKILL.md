...
description: Methodology for DFCode management usage reports and analysis — including "查查今日用量"/"今日用量怎么样", department and employee token usage, same-window today-vs-yesterday comparison, model structure, root-cause breakdown, and usage-decline alerts. Read this skill before any DFCode tool call for organization-level today-usage questions; a bare total is never a complete answer.
---
...
## 0.1 Department/project name resolution (mandatory before scoped queries)

The user's department or project term may be abbreviated, omit suffixes such as `部/项目/中心`, or contain a one-character typo. **Never use the raw user term as the only exact filter and conclude "no usage" from a zero result.**

用户说的部门或项目名可能是简称、少了 `部/项目/中心` 等后缀，或有一字误写。**禁止把用户原词直接作为唯一精确过滤条件**，再根据 0 行或 0 token 断言没有用量。

1. Unless the user explicitly identifies the dimension, resolve it as `auto`: retrieve broad department and project candidates without an exact name filter.
2. Normalize Unicode/case, spaces, punctuation, and common suffixes; rank exact, normalized, containment/abbreviation, then explainable one-character matches.
3. Use `scripts/dimension_match.py` for deterministic matching when needed. Continue only for one high-confidence `dimension + canonical name` result.
4. Disclose the mapping: `名称匹配: 用户输入「智能视迅」→ 维度「部门」→ 规范名「智能视讯项目部」(一字近似)`.
5. If candidates remain ambiguous, ask the user to choose. If unmatched, report `未匹配名称: <原词>` separately; never turn it into a zero-usage claim.
6. If an initial exact query returns zero, perform this candidate fallback across both departments and projects before answering；在未确认真实维度和规范名之前，**不能结束查询**。
...
2. **Run the engine — never hand-compute** (§8's rule applies here too):
   - Use `write_file` to save the complete payload as `/mnt/user-data/tmp/dfcode_usage_cube_input.json`.
   - Then use `bash` with this short command:
   ```bash
   python3 /mnt/skills/agent/dfcode-usage-analysis/scripts/usage_cube.py --intraday --md < /mnt/user-data/tmp/dfcode_usage_cube_input.json
   ```
   Do not embed the JSON in `echo`, a heredoc, or `python -c`: the shell command has a strict size budget and bundled skills reject inline interpreters. The `write_file` call must contain the full valid JSON object.
...
A bare total (or a top-users list) is an incomplete answer to `查查今日用量`, "今日 DFCode 用量怎么样/升了还是降了", and equivalent organization-level questions. The reader is a manager: they need **which departments**, **which people**, and **which models** moved. Required composition (~4-6 calls, within budget):

1. `query_usage {from: 昨日, to: 今日, groupBy: "hour_day"}` → org same-window headline (§3b).
2. **Per-user, STRICT same-window (preferred)** — `query_usage` supports `hourTo` (exclusive upper hour bound, Asia/Shanghai, composable with every groupBy):
   `query_usage {from: 昨日, to: 昨日, groupBy: "user", hourTo: H}` + `query_usage {from: 今日, to: 今日, groupBy: "user", hourTo: H}` (H = current hour) → each user's tokens for the SAME hours-[0,H) window on both days. 2 small calls.
3. Map user → department (`query_departments` or `query_roster`, 1 call); **drop unassigned people entirely** (§1).
4. Fetch model structure for today and the comparison window (`query_usage` grouped by `model`, or `query_dashboard` only as supporting context). Include the top models with token share and call out meaningful mix shifts when the data supports them.
5. Feed everything into ONE engine run — hourly rows as `items`, per-user windowed rows as `users` with `users_are_windowed: true`. Save this object with `write_file` to `/mnt/user-data/tmp/dfcode_usage_cube_input.json`, then execute the short engine command from §3b:
   ```json
   {"items": [...hour_day...], "cutoff_hour": 16, "users_are_windowed": true,
    "users": [{"name": "张三", "department": "智能视迅", "baseline_tokens": 33200000, "today_tokens": 16050000}, ...]}
   ```
   (`baseline_tokens` = yesterday's WINDOWED figure from step 2.) The engine compares today-window vs baseline-window directly and rolls up per department, classifying drop / rise / silent_today / new.
   **Fallback (older server without `hourTo`)**: fetch full-day per-user totals instead, omit `users_are_windowed` — the engine then scales yesterday-full-day by the org pace ratio into an expected-by-now **estimate** (labeled 估算 in gaps/caption automatically).
6. **Mandatory answer structure** (in this order):
   ```
   结论:总量同窗 Δ±X%(均截至 HH:00)，主要由 <部门/人员/模型> 驱动。
   核心指标:<今日 tokens、请求数、活跃人数、峰值时段、全天预测(推测)>
   分部门(同窗期望 vs 实际):<每部门一行,降幅大的在前>
   📉 明显下降:<人·部门:期望→实际(Δ-X% / 今日未使用)>
   📈 明显上升:<人·部门:…>(有新增用户也点出)
   模型结构:<Top 模型 tokens + 占比;显著迁移如有>
   诊断与建议:<2-4 条直接可执行的管理判断/动作>
   口径:同窗;仅已分配部门;用量按 token 计。
   缺口:<engine gaps 原样带出>
   ```
   Never answer with only total/request count/yesterday comparison/peak hour or only a "谁用得多" ranking; never present the drop list computed as "today-partial vs yesterday-full" per person (that marks everyone as fake-dropping).
...
Flatten whatever the MCP returned into the cube's `records` list and run it through the engine. **Do the arithmetic in the engine, not in context.** Use `write_file` for the JSON payload so large data never enters the shell command.

```bash
python3 /mnt/skills/agent/dfcode-usage-analysis/scripts/usage_cube.py --md < /mnt/user-data/tmp/dfcode_usage_cube_input.json
